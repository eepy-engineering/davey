import * as chai from 'chai';
import chaiAsPromised from 'chai-as-promised';
chai.use(chaiAsPromised);
import 'mocha';
import { CipherSuite } from '../src/util/constants';
import { CipherSuiteInterface } from '../src/util/ciphersuite';
const expect = chai.expect;

describe('signWithLabel', () => {
  it('should sign properly', async () => {
    const data = new Uint8Array([
      , 
    ]);

    const ciphersuite = new CipherSuiteInterface(CipherSuite.MLS_128_DHKEMP256_AES128GCM_SHA256_P256);
    const { privateKey, publicKey } = await ciphersuite.generateSigningKeyPair();
    const signature = await ciphersuite.signWithLabel(privateKey, 'LeafNodeTBS', data);
    expect(signature).to.be.a('Uint8Array');
    expect(ciphersuite.verifyWithLabel(publicKey, 'LeafNodeTBS', signature, data)).to.eventually.equal(true);
  });
});