...
export type Algorithm = crypto.webcrypto.AlgorithmIdentifier | crypto.webcrypto.RsaPssParams | crypto.webcrypto.EcdsaParams | crypto.webcrypto.Ed448Params;
...
export class CipherSuiteInterface extends CipherSuite {
  ...
  generateKeyPair() {
    const pk = p256.utils.randomPrivateKey();
    return {
      privateKey: pk,
      publicKey: p256.getPublicKey(pk)
    };
  }
  ...
  async #sign(key: Uint8Array, message: Uint8Array) {
    return secp256r1.sign(message, key).toCompactRawBytes();;
  }

  async #verify(key: Uint8Array, message: Uint8Array, signature: Uint8Array) {
  ...
  async verifyWithLabel(publicKey: Uint8Array, label: string, signature: Uint8Array, content: Uint8Array) {
    const signContent = serializeResolvers([
      ['v', Buffer.from(LABEL_HEADER + label)], // label
      ['v', content]                            // content
    ]);
    return await this.#verify(publicKey, signContent, signature);
  }

  /** @see https://www.rfc-editor.org/rfc/rfc9420.html#section-5.1.2 */
  async signWithLabel(privateKey: Uint8Array, label: string, content: Uint8Array) {
    const signContent = serializeResolvers([
      ['v', Buffer.from(LABEL_HEADER + label)], // label
      ['v', content]                            // content
    ]);
    return this.#sign(privateKey, signContent);
  }
  
  /** @see https://www.rfc-editor.org/rfc/rfc9420.html#section-5.1.3 */
  async encryptWithLabel(publicKey: Uint8Array, label: string, context: Uint8Array, plaintext: Uint8Array) {
    const encryptContext = serializeResolvers([
      ['v', Buffer.from(LABEL_HEADER + label)], // label
      ['v', context]                            // context
    ]);
    const key = await this.kem.deserializePublicKey(publicKey.buffer as ArrayBuffer);
    return this
      .seal(
        {
  ...
  async decryptWithLabel(privateKey: Uint8Array | Key, label: string, context: Uint8Array, kemOutput: Uint8Array, ciphertext: Uint8Array) {
    ...
    const key = privateKey instanceof Uint8Array ? await this.kem.deserializePrivateKey(privateKey.buffer as ArrayBuffer) : privateKey;
    return this
      .open(
        {
          recipientKey: key,
